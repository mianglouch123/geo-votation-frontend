// presentation/pages/votation/UpdateVotation.jsx
import { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { UseUserProfile } from '../../hooks/user/UseUserProfile.jsx';
import { UseUpdateVotation } from '../../hooks/votation/UseUpdateVotation.jsx';
...
export default function UpdateVotation() {
  const navigate = useNavigate();
  const { id } = useParams();
  const { user: dataUser, loading: loadingUserId } = UseUserProfile();
  const { execute: updateVotation, loading: updating } = UseUpdateVotation();
  const { data: votationData, loading: loadingVotation } = UseVotation(id);
  ...
  useEffect(() => {
    if (votationData && dataUser) {
      setFormData({
        votationId: id,
        ownerId: dataUser?.id,
        subject: votationData.subject || '',
        description: votationData.description || '',
        closes_at: votationData.closes_at?.split('T')[0] || '',
        questions: votationData.questions?.map(q => ({
          id: Date.now().toString() + '-' + Math.random().toString(36).substr(2, 6) + '-' + q._id,
          questionId: q._id,
          label: q.label || '',
          code: q.code || '',
          type: q.type || 'SHORTANSWER',
          isRequired: q.isRequired || false,
          config: q.config?.config || {}
        })) || []
      });
    }
  }, [votationData, dataUser, id]);

  if (loadingUserId || loadingVotation) {
    return (
  ...
  const handleSubmit = async (e) => {
    ...
    // Parsear preguntas para el backend (eliminar id, usar questionId)
    const questionsForPayload = formData.questions.map(q => {
      ...
      // ✅ Solo incluir questionId si existe (pregunta existente)
      if (q.questionId) {
        questionData.questionId = q.questionId;
      }
      // ✅ Para preguntas nuevas, NO incluir questionId
    ...
    try {
      const result = await updateVotation(payload);
      setTimeout(() => {
        alert("Votación actualizada correctamente");
        navigate(`/dashboard/votations/${id}`);
      }, 500);
    } catch (err) {
      console.error('Error updating votation:', err);
      alert(err.message || 'Error al actualizar la votación');
    }
  };
      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-r from-purple-600 to-purple-800 rounded-2xl p-8 text-white shadow-xl">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold">Editar votación</h1>
                <p className="text-purple-100 text-sm">Modifica los datos de tu votación</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            ...
            >
              {updating ? 'Actualizando...' : 'Actualizar votación'}
            </button>
          </div>
        </form>
      </div>
