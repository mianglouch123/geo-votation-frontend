...
export default function PublicAnswerVotationView() {
  ...
  // Inicializar formData cuando se carga la votación
  useEffect(() => {
    if (votationData?.questions) {
      const initialFormData = votationData.questions.map(q => ({
        questionId: q.id,
        questionVersion: q.version,
        value: initializeValueByType(q.type, q.config)
      }));
      setFormData(initialFormData);
    }
  }, [votationData]);

  // Inicializar valor según tipo de pregunta
  const initializeValueByType = (type, config) => {
    switch (type) {
      case 'SHORTANSWER':
      case 'LARGEANSWER': {
        return { value: '' };
      ...
      case 'DATE': {
        return { date: config?.date || new Date().toISOString().split('T')[0] };
      }
      
      case 'HOUR': {
        const now = new Date();
        return {
          hour: config?.hour || now.getHours().toString().padStart(2, '0'),
          min: config?.min || now.getMinutes().toString().padStart(2, '0')
        };
      }
  ...
  useEffect(() => {
    if (votationData?.votation?.isOpen === false) {
      navigate('/dashboard');
    }
  }, [votationData, navigate]);
  ...
  const validateForm = () => {
    ...
    const questionsMap = new Map(
      votationData.questions.map(q => [q.id, q])
    );
    ...
    formData.forEach(item => {
      ...
      // Validar MULTI_OPTION (solo una opción si no permite múltiple)
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-r from-purple-600 to-purple-800 rounded-2xl p-8 text-white shadow-xl">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold">{votationData.votation?.subject}</h1>
                <p className="text-purple-100 text-sm">{votationData.votation?.description}</p>
              </div>
            </div>
            <div className="mt-4 text-sm text-purple-200 border-t border-purple-500/30 pt-3 flex items-center gap-2">
              ...
              <span>Cierra: {new Date(votationData.votation?.closes_at).toLocaleDateString()}</span>
            </div>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          {votationData.questions?.map((question, idx) => {
